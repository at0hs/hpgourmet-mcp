// HotPepper Web Service (Recruit) の各APIエンドポイントのUI向け定義。
// hotpepper_api_console のフォームUI生成・プロキシ側のパラメータホワイトリスト検証の単一ソースとして使う。
// key(APIキー)とformatはサーバー側で強制するためここには含めない。
// エンドポイントのslug一覧（＝Recruit側のURLパスセグメント）自体は../hotpepper/endpointsが正典。
import type { HotpepperEndpointSlug } from '../hotpepper/endpoints';
...
export const ENDPOINTS: Record<HotpepperEndpointSlug, EndpointDef> = {
  gourmet: {
    label: 'グルメサーチAPI',
  ...
  special_category: {
    ...
    params: [
      { name: 'special_category', label: '特集カテゴリコード', type: 'text', placeholder: 'SPC0', group: '基本', hint: '完全一致。複数可' },
    ],
  },
};
