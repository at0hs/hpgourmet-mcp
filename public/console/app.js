...
function selectApi(slug) {
  ...
  $('#brand-slug').textContent = slug + '/v1';
  renderForm();
  renderWire();
}
...
function renderField(def) {
  ...
  const lab = el('label');
  lab.innerHTML =
    esc(def.label) + (def.required ? ' <span style="color:var(--accent)">*</span>' : '') + '<span class="pn">' + esc(def.name) + '</span>';
  row.appendChild(lab);
...
function renderTopLevelBlock(obj) {
  ...
  if (metaEntries.length) {
    html += '<div class="meta-bar">' + metaEntries.map(([k, v]) => '<b>' + esc(k) + '</b> ' + esc(v)).join('') + '</div>';
  }
...
function highlight(s) {
  return esc(s).replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+-]?\d+)?)/g, (m) => {
    let cls = 'j-num';
